<script setup lang="ts">
import type { AdminCategoryItem } from '#shared/types/admin'

definePageMeta({ layout: 'admin', middleware: 'admin' })
defineI18nRoute(false)
useSeoMeta({ title: '分类管理', robots: 'noindex, nofollow' })

const { data, pending, error, refresh } = await useFetch<{ categories: AdminCategoryItem[] }>(
  '/api/admin/categories',
  {
    key: 'admin-categories-page',
  },
)
const categories = computed(() => data.value?.categories ?? [])
const categoryRows = computed(() => {
  const childrenByParent = new Map<string | null, AdminCategoryItem[]>()
  for (const category of categories.value) {
    const siblings = childrenByParent.get(category.parentId) ?? []
    siblings.push(category)
    childrenByParent.set(category.parentId, siblings)
  }

  const rows: Array<{ category: AdminCategoryItem; depth: number }> = []
  const visited = new Set<string>()
  const visit = (parentId: string | null, depth: number) => {
    for (const category of childrenByParent.get(parentId) ?? []) {
      if (visited.has(category.id)) continue
      visited.add(category.id)
      rows.push({ category, depth })
      visit(category.id, depth + 1)
    }
  }

  visit(null, 0)
  for (const category of categories.value) {
    if (!visited.has(category.id)) rows.push({ category, depth: 0 })
  }

  return rows
})
const editingId = ref<string | null>(null)
const saving = ref(false)
const notice = ref('')
const errorMessage = ref('')
const form = reactive({ name: '', slug: '', description: '', parentId: '' })

function resetForm() {
  editingId.value = null
  form.name = ''
  form.slug = ''
  form.description = ''
  form.parentId = ''
  errorMessage.value = ''
}

function editCategory(category: AdminCategoryItem) {
  editingId.value = category.id
  form.name = category.name
  form.slug = category.slug
  form.description = category.description ?? ''
  form.parentId = category.parentId ?? ''
  errorMessage.value = ''
}

function getRequestMessage(error: unknown) {
  const requestError = error as {
    statusMessage?: string
    data?: { statusMessage?: string }
  }
  return requestError.statusMessage ?? requestError.data?.statusMessage ?? ''
}

async function submit() {
  if (saving.value) return
  if (!form.name.trim() || !form.slug.trim()) {
    errorMessage.value = '分类名称和 slug 不能为空。'
    return
  }

  saving.value = true
  errorMessage.value = ''
  try {
    if (editingId.value) {
      await $fetch(`/api/admin/categories/${editingId.value}`, {
        method: 'PATCH',
        body: { ...form, parentId: form.parentId || null },
      })
      notice.value = '分类已更新。'
    } else {
      await $fetch('/api/admin/categories', {
        method: 'POST',
        body: { ...form, parentId: form.parentId || null },
      })
      notice.value = '分类已创建。'
    }
    resetForm()
    await refresh()
  } catch (requestError) {
    const statusCode = (requestError as { statusCode?: number }).statusCode
    const statusMessage = getRequestMessage(requestError)
    if (statusCode === 409) errorMessage.value = '分类 slug 已存在。'
    else if (/cycle/i.test(statusMessage)) errorMessage.value = '父分类关系不能形成循环。'
    else if (/parent/i.test(statusMessage)) errorMessage.value = '所选父分类不存在，请刷新后重试。'
    else errorMessage.value = '分类名称或 slug 格式有误，请检查后重试。'
  } finally {
    saving.value = false
  }
}

async function remove(category: AdminCategoryItem) {
  if (!import.meta.client || !window.confirm(`确定删除分类“${category.name}”吗？文章不会被删除。`))
    return
  try {
    await $fetch(`/api/admin/categories/${category.id}`, { method: 'DELETE' })
    if (editingId.value === category.id) resetForm()
    notice.value = '分类已删除。'
    await refresh()
  } catch {
    errorMessage.value = '删除分类失败，请稍后重试。'
  }
}
</script>

<template>
  <div class="space-y-4 sm:space-y-5">
    <section>
      <p class="mb-1.5 text-[10px] font-bold tracking-[1.35px] text-[#7f8ba0]">ARTICLE STRUCTURE</p>
      <h1
        class="text-[25px] font-bold leading-tight tracking-[-0.3px] text-[#1a2233] max-sm:text-[23px]"
      >
        分类管理
      </h1>
      <p class="mt-1.5 text-[13px] text-[#7a8699]">用层级结构整理文章。</p>
    </section>
    <p
      v-if="notice"
      class="rounded-lg border border-[#b7d5ff] bg-[#eaf2ff] px-3.5 py-2.5 text-xs text-[#1554a3]"
      role="status"
    >
      {{ notice }}
    </p>
    <div
      v-if="error"
      class="rounded-xl border border-[#ffc9c9] bg-[#fff0f0] px-4 py-3 text-xs text-[#cf1322]"
      role="alert"
    >
      暂时无法加载分类列表。
    </div>

    <section class="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(280px,0.7fr)_minmax(0,1.7fr)]">
      <form
        class="rounded-[14px] border border-[#e8edf3] bg-white p-4 shadow-[0_8px_24px_rgba(16,24,40,0.045)] sm:p-5"
        @submit.prevent="submit"
      >
        <div class="flex items-center justify-between gap-3">
          <div>
            <h2 class="text-[15px] font-semibold text-[#1a2233]">
              {{ editingId ? '编辑分类' : '新增分类' }}
            </h2>
            <p class="mt-1.5 text-[11px] text-[#7a8699]">分类 slug 用于公开归档地址。</p>
          </div>
          <button
            v-if="editingId"
            type="button"
            class="text-[11px] text-[#7a8699] hover:text-[#1677ff]"
            @click="resetForm"
          >
            取消编辑
          </button>
        </div>
        <div class="mt-5 space-y-4">
          <div>
            <label for="category-name" class="mb-2 block text-xs font-semibold text-[#657286]"
              >名称</label
            ><input
              id="category-name"
              v-model="form.name"
              type="text"
              maxlength="80"
              placeholder="例如：产品方法"
              class="h-10 w-full rounded-lg border border-[#e8edf3] bg-[#fbfcfe] px-3 text-xs outline-none focus:border-[#9cc2ff] focus:bg-white focus:ring-4 focus:ring-[#1677ff]/10"
            />
          </div>
          <div>
            <label for="category-slug" class="mb-2 block text-xs font-semibold text-[#657286]"
              >slug</label
            ><input
              id="category-slug"
              v-model="form.slug"
              type="text"
              maxlength="200"
              placeholder="例如：product"
              class="h-10 w-full rounded-lg border border-[#e8edf3] bg-[#fbfcfe] px-3 text-xs outline-none focus:border-[#9cc2ff] focus:bg-white focus:ring-4 focus:ring-[#1677ff]/10"
            />
          </div>
          <div>
            <label for="category-parent" class="mb-2 block text-xs font-semibold text-[#657286]"
              >父分类 <span class="font-normal text-[#a2adbd]">（可选）</span></label
            ><select
              id="category-parent"
              v-model="form.parentId"
              class="h-10 w-full rounded-lg border border-[#e8edf3] bg-[#fbfcfe] px-3 text-xs outline-none focus:border-[#9cc2ff] focus:bg-white focus:ring-4 focus:ring-[#1677ff]/10"
            >
              <option value="">顶级分类</option>
              <option
                v-for="category in categories"
                v-show="category.id !== editingId"
                :key="category.id"
                :value="category.id"
              >
                {{ category.parentId ? '　└ ' : '' }}{{ category.name }}
              </option>
            </select>
          </div>
          <div>
            <label
              for="category-description"
              class="mb-2 block text-xs font-semibold text-[#657286]"
              >描述 <span class="font-normal text-[#a2adbd]">（可选）</span></label
            ><textarea
              id="category-description"
              v-model="form.description"
              maxlength="500"
              rows="3"
              placeholder="简短描述这个分类"
              class="w-full resize-y rounded-lg border border-[#e8edf3] bg-[#fbfcfe] px-3 py-2.5 text-xs leading-5 outline-none focus:border-[#9cc2ff] focus:bg-white focus:ring-4 focus:ring-[#1677ff]/10"
            />
          </div>
          <button
            type="submit"
            class="inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-[#1677ff] text-xs font-semibold text-white shadow-[0_5px_12px_rgba(22,119,255,0.2)] hover:bg-[#0d69e8] disabled:opacity-60"
            :disabled="saving"
          >
            <Icon
              :name="saving ? 'lucide:loader-circle' : 'lucide:check'"
              class="size-4"
              :class="saving ? 'animate-spin' : ''"
            />{{ saving ? '保存中…' : editingId ? '保存修改' : '创建分类' }}
          </button>
          <p
            v-if="errorMessage"
            class="rounded-lg bg-[#fff0f0] px-3 py-2 text-xs leading-5 text-[#cf1322]"
            role="alert"
          >
            {{ errorMessage }}
          </p>
        </div>
      </form>

      <article
        class="min-w-0 rounded-[14px] border border-[#e8edf3] bg-white p-4 shadow-[0_8px_24px_rgba(16,24,40,0.045)] sm:p-5"
      >
        <div class="flex items-center justify-between gap-4">
          <div>
            <h2 class="text-[15px] font-semibold text-[#1a2233]">分类列表</h2>
            <p class="mt-1.5 text-[11px] text-[#7a8699]">共 {{ categories.length }} 个分类</p>
          </div>
          <Icon name="lucide:folder-tree" class="size-4 text-[#b0bac6]" />
        </div>
        <div class="mt-4 overflow-x-auto">
          <table class="min-w-[560px] w-full border-collapse text-left text-[11px]">
            <thead>
              <tr class="border-b border-[#eef1f5] text-[10px] font-medium text-[#9aa5b4]">
                <th class="px-2 py-3">分类名称</th>
                <th class="px-2 py-3">slug</th>
                <th class="px-2 py-3">文章数</th>
                <th class="px-2 py-3 text-right">操作</th>
              </tr>
            </thead>
            <tbody>
              <template v-if="pending"
                ><tr>
                  <td colspan="4" class="px-2 py-12 text-center text-[#a2adbd]">加载中…</td>
                </tr></template
              ><template v-else-if="!categories.length"
                ><tr>
                  <td colspan="4" class="px-2 py-12 text-center text-[#a2adbd]">暂无分类</td>
                </tr></template
              ><template v-else
                ><tr
                  v-for="row in categoryRows"
                  :key="row.category.id"
                  class="border-b border-[#f0f2f6] last:border-0"
                >
                  <td class="px-2 py-3 text-[#526074]">
                    <span
                      class="mr-2 inline-flex items-center text-[#a2adbd]"
                      :style="{ paddingLeft: `${row.depth * 20}px` }"
                      ><Icon
                        :name="row.depth ? 'lucide:corner-down-right' : 'lucide:folder'"
                        class="mr-1.5 size-3.5"
                        :class="row.depth ? 'text-[#a2adbd]' : 'text-[#fa8c16]'"
                      /> </span
                    ><strong :class="row.depth ? 'font-normal' : 'font-semibold'">{{
                      row.category.name
                    }}</strong>
                  </td>
                  <td class="px-2 py-3 text-[#7e8998]">{{ row.category.slug }}</td>
                  <td class="px-2 py-3 tabular-nums text-[#59677a]">
                    {{ row.category.contentCount }}
                  </td>
                  <td class="px-2 py-3 text-right">
                    <button
                      type="button"
                      class="mr-1 rounded-md px-2 py-1.5 text-[10px] text-[#1677ff] hover:bg-[#eaf2ff]"
                      @click="editCategory(row.category)"
                    >
                      编辑</button
                    ><button
                      type="button"
                      class="rounded-md px-2 py-1.5 text-[10px] text-[#f5222d] hover:bg-[#fff0f0]"
                      @click="remove(row.category)"
                    >
                      删除
                    </button>
                  </td>
                </tr></template
              >
            </tbody>
          </table>
        </div>
      </article>
    </section>
  </div>
</template>
